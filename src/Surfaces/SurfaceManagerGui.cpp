...
void SurfaceManagerGui::draw(){
	...
	if(guiMode == GuiMode::NONE){
		...
		sourcesEditor.draw();
	}else if(guiMode == GuiMode::PLAYLIST_SELECTION_MODE){
		surfaceManager->draw();
		sourcesEditor.draw();
	}
}
...
void SurfaceManagerGui::setMode(int newGuiMode){
	if(newGuiMode != GuiMode::NONE && newGuiMode != GuiMode::TEXTURE_MAPPING &&
	   ...
	   newGuiMode != GuiMode::PLAYLIST_SELECTION_MODE){
	...
	if(guiMode == GuiMode::PLAYLIST_SELECTION_MODE){
		surfaceManager->selectSurface(0);
		sourcesEditor.enable();
	}else{
		sourcesEditor.disable();
	}
