...
void RadioList::setup(vector <string> & labels, vector <string> & values){
	...
	for(i = 0; i < labels.size(); i++){
		...
		toggle->setup(false);
		toggle->loadFont("Roboto-Black.ttf",25, true);
		toggle->setBackgroundColor(ofColor(255,255,255,100));
		toggle->setDefaultHeight(50);
		toggle->setDefaultWidth(750);
		toggle->setDefaultBackgroundColor(ofColor(119,119,119,220));
...
void RadioList::clearColors() {
	for(int i = 0; i < guiGroup.getNumControls(); i++){
			ofxToggle * toggle = static_cast <ofxToggle *>(guiGroup.getControl(i));
			toggle->setBackgroundColor(ofColor(119,119,119,220));
	}
}
